...
from core.agent.config import BATCH_COMMIT_SIZE, DEFAULT_CHAT_MODEL, FEATURE_FLAGS
from core.agent.payroll_lines import build_payroll_lines
from core.agent.state import PayslipContext, ProcessingState, VidaLaboralContext
...
class ValeriaAgent:
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            ...
                            "last_month": {
                                "type": "string",
                                "description": "Optional cutoff month in MM/YYYY format (e.g., '05/2024'). Missing payslips will only be calculated up to this month, not beyond."
                            }
                        },
                        "required": []
                    }
    ...
    def _list_employees_for_company(self, company_id: str, cutoff_date=None):
        """
        List all employees for a company from either production or local database.

        Args:
            company_id: The company ID to filter by
            cutoff_date: Optional date to filter employees. Only includes employees
                        who were employed on or before this date.
        ...
        if self.use_production_data and self.prod_session:
            ...
            for prod_emp in prod_employees:
                ...
                if cutoff_date:
                    # Skip if employee started after cutoff
                    if prod_emp.begin_date and prod_emp.begin_date > cutoff_date:
                        continue
                    # Skip if employee ended before their start (shouldn't happen, but safety check)
                    if prod_emp.end_date and prod_emp.begin_date and prod_emp.end_date < prod_emp.begin_date:
                        continue
    ...
    def process_vida_laboral_csv(self, file_path: str, client_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        """
        context = VidaLaboralContext(processing_state=self.processing_state)
        ...
        try:
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    vida_laboral.process_row(self.session, client.id, row, context)

            self.session.commit()
            self.processing_state.vida_laboral_processed = True
            self.processing_state.employees_created = context.employees_created
            ...
            return {
                ...
                "message": f"Created {context.employees_created} employment periods, updated {context.employees_updated}, created {context.vacation_periods_created} vacation periods for {client_name}"
            }
    ...
    def detect_missing_payslips(self, client_id: Optional[str] = None, last_month: Optional[str] = None) -> Dict[str, Any]:  # Updated type hint
        """
        ...
        try:
            ...
            if not client_id:
                client_id = self.processing_state.client_id
                if not client_id:
                    return {
                        "success": False,
    ...
    def generate_missing_payslips_report(self, client_id: Optional[uuid.UUID] = None,
                                       output_format: str = "json",
                                       save_to_file: bool = False,
                                       filename: Optional[str] = None,
                                       last_month: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            result = self.detect_missing_payslips(client_id, last_month=last_month)
    ...
    def process_payslip_batch(self, pdf_files: List[str], batch_size: int = 50) -> Dict[str, Any]:
        ...
        Args:
            ...
            batch_size: [DEPRECATED - No longer used] Each payroll is now committed immediately after processing
        ...
        print(f"💾 Each payroll will be committed immediately after processing")
                ...
                try:
                    ...
                    for emp_info in process_payslip(pdf_file, session=self.session):
                        ...
                        try:
                            ...
                            trabajador = emp_info.get("trabajador", {})
                            name = trabajador.get("nombre", "").strip()
                            emp_id = trabajador.get("dni", "").strip()

                            if not name and not emp_id:
                                print(f"⚠️  SKIPPING: Empty extraction (no name and no ID)")
                                period_label = format_periodo(emp_info.get('periodo'))
                            ...
                            if employee:
                                ...
                                created_lines = build_payroll_lines(
                                    session=self.session,
                                    payroll_id=payroll.id,
                                    collections=line_collections,
                                )
                                line_count = len(created_lines)

                                # Immediate commit: Save each payroll right after processing
                                try:
                                    self.session.commit()
                                    processed_count += 1
                                except Exception as commit_error:
                                    print(f"\n⚠️  Failed to commit payroll for {pdf_file}: {commit_error}")
                                    print(f"   Rolling back this record...")
                                    self.session.rollback()
                                    # Record will not be counted as processed
                                    failed_count += 1
                                    results.append({
                                ...
                                period_label = format_periodo(periodo_data)
                                results.append({
                            ...
                            # Skip this payroll - other payrolls are already committed individually
                            failed_count += 1
                            results.append({
                                "employee": emp_info.get('name', 'Unknown'),
                                "file": pdf_file,
                    ...
                    # After iterating through all yielded payrolls from this PDF
        ...
        print(f"   💾 Each payroll committed immediately after processing")
        ...
        return {
            ...
            "message": f"Processed {processed_count} payslips with complete data extraction, {failed_count} failed (took {total_time:.1f}s). Each payroll was committed immediately after processing."
        }

    def _load_concept_mappings(self) -> Dict[str, Dict]:
        """Load all concept mappings from database for intelligent matching"""
        if not self.feature_flags.enable_concept_mapping:
            return {}
    ...
    def _find_matching_employee(self, emp_info: Dict) -> Optional[Employee]:
        ...
        if not client_id:
            empresa = emp_info.get("empresa", {})
            company_cif = (empresa.get("cif") or "").strip()
            company_name = (empresa.get("razon_social") or "").strip()
    ...
    def list_payrolls(self, employee_id: Optional[int] = None, year: Optional[int] = None,
        ...
        try:
            ...
            filtered_payrolls = []
            for payroll in payrolls:
                ref_date = period_reference_date(payroll.periodo)
...
**Detect MODE 1 (File Processing) when user:**
  ...
  – Wants to generate a report on a company
...
**User:** "Generate a report for company ID eea29097-940c-4d2c-a598-59a0be40a78b"
**You:** [call generate_missing_payslips_report with client_id=eea29097-940c-4d2c-a598-59a0be40a78b, last_month=01/2025, output_format="json", save_to_file=True and filename="tmp_report.json"] → Present report
...
# CURRENT STATE
"""f"""
Processing State: {self.processing_state.to_dict()}
