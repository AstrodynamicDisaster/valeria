...
class OpenAIPayslipParser(BasePayslipParser):
    ...
    def parse_with_usage_info(self, pdf_bytes: bytes, text_pdf: str = "") -> Tuple[str, Dict]:
        ...
        user_parts = [
            {"type": "input_text", "text": "I have this payslip in PDF, can you process it?"},
            {"type": "input_text", "text": f"This is the text of the payslip in raw (for you to help): ```{text_pdf}```"},
            {
                "type": "input_file",
...
class GeminiPayslipParser(BasePayslipParser):
    ...
    def __init__(
        ...
        if not GEMINI_AVAILABLE:
            raise ImportError("google-genai package is required for GeminiPayslipParser")
    ...
    def parse_with_usage_info(self, pdf_bytes: bytes, text_pdf: str = "") -> Tuple[str, Dict]:
        ...
        contents = [
            types.Content(
                role="user",
                parts=[
                    types.Part.from_text(text="I have this payslip in PDF, can you process it?"),
                    types.Part.from_text(text=f"This is the text of the payslip in raw (for you to help): ```{text_pdf}```"),
                    pdf_part
                ]
            ),
        ]
